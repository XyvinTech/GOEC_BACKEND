...
app.get('/api/health-check',((req, res) =>{
  res.status(200).send('connected to configuration-service api!!')
}))
...
//main API
app.use(logger('dev'))
app.use('/api/v1', authVerify, configurationRoutes)

// 404 
app.all('*', (req, res, next) => {
    const err = new createError(
      404,
      `Cant find the ${req.originalUrl} on the configuration service server !`
    )
    next(err)
  })
