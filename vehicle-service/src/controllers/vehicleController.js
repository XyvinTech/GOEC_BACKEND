const { vehicleValidationSchema, vehicleUpdateValidationSchema } = require('../validation/vehicleValidationSchema');
...
const createVehicle = async (req, res) => {
    ...
    try {
        ...
        let { error, value } = vehicleValidationSchema.validate(req.body)
        if (error) {
...
const getAllVehicles = async (req, res) => {
    ...
    const result = brands.map(brand => {
        const brandWiseVehicles = allVehicles.filter(vehicle => String(vehicle.brand) === String(brand._id))
...
const getAllVehiclesForDashboard = async (req, res) => {
    ...
    const allVehicleData = await Vehicle.aggregate(vehiclePipeline).skip(10*(pageNo-1)).limit(10);
    ...
    const brandMap = new Map(brandInfo.map(brand => [String(brand._id), brand.brandName]));
...
const getVehicleById = async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) throw new createError(400, `Invalid id ${id}`);
...
const getVehiclesByIds = async (req, res) => {
    ...
    if (!Array.isArray(idArray)) throw new createError(400, "idArray should be array")

    const vehicles = await Vehicle.find({ _id: { $in: idArray } });
    const brandResult = await Brand.find({ _id: { $in: vehicles.map(x => x.brand) } });

    const result = vehicles.map(vehicle => {
        const brandFound = brandResult.find(brand => brand._id.toString() == vehicle.brand.toString())
...
module.exports = { createVehicle, imageUploadAlone, getAllVehiclesForDashboard, getAllVehicles, getVehicleById, updateVehicleById, deleteVehicleById, getVehiclesByIds }