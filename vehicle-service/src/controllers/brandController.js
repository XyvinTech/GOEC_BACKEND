...
const createBrand = async (req, res) => {
    if (!req.body.brandName) throw new createError(400, 'brandName field is required')
    let value = req.body
...
const getAllBrands = async (req, res) => {
    ...
    const brands = await Brand.find(filter).skip(10*(pageNo-1)).limit(10);
    let totalCount = await Brand.find(filter).countDocuments()
    const formattedBrands = brands.map(brand => ({
        ...brand._doc,
        // Assuming 'createdAt' is the field containing the date
        createdAt: moment(brand.createdAt).format('DD-MM-YYYY '), // Adjust the format as needed
    }));
    res.status(200).json({
        status: true,
        message:'OK',
...
module.exports = { createBrand, getAllBrands, updateBrandById, deleteBrandById, getAllBrandsDropdown };