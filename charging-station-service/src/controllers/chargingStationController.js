...
const {
  ...
  chargingStationEditSchema,
  addRatingSchema,
} = require('../validation/chargingStationSchema')
...
const {
  getChargerDetails,
  deleteChargers,
} = require('../services/evMachineServiceApis')
// @ts-ignore
...
exports.getChargingStationById = async (req, res) => {
  ...
  try {
    // @ts-ignore
    userApiResponse = await axios.get(
    ...
    pipedData = await ChargingStation.aggregate([
...

      {
        $lookup: {
          ...
          pipeline: [
            // Add this pipeline to project only required fields
            {
              $project: {
    ...
    chargers_output = await Promise.all(
  ...
  const chargingStationList = await ChargingStation.aggregate([
...

    {
      $group: {
        ...
        connectorsType: {
          $push: '$evMachines.evModelDetails.connectors.type',
        },
      },
    },
    {
...
exports.inbetweenPointsList = async (req, res) => {
  ...
  if (!start || !end) {
    return res
      ...
      .json({
        success: false,
        message: 'Please provide both start and end coordinates.',
      })
  }
  ...
  try {
    ...
    const mappedData = await ChargingStation.aggregate([
      {
        $match: {
          location: {
            $geoWithin: {
      ...
      {
        $unwind: {
          path: '$chargers.connectors',
          preserveNullAndEmptyArrays: true,
        },
      },
      {
        $addFields: {
          'chargers.connectorStatus': {
            $switch: {
              branches: [
                {
                  case: { $eq: ['$chargers.connectors.status', 'Available'] },
                  then: 'Online',
                },
                {
                  case: { $eq: ['$chargers.connectors.status', 'Unavailable'] },
                  then: 'Offline',
                },
                {
                  case: { $in: ['$chargers.connectors.status', ['Preparing', 'Charging', 'Finishing']] },
                  then: 'Busy',
                },
              ],
              default: 'Busy',
      ...
      },
      {
        $group: {
          ...
          chargers: {
            $push: {
              status: '$chargers.connectorStatus',
              evModel: '$chargers.evModel',
            },
          },
          name: { $first: '$name' },
      ...
      },
      {
        $addFields: {
          charger_status: {
            $cond: {
              if: { $in: ['Busy', '$chargers.status'] },
              then: 'Busy',
              else: {
                $cond: {
                  if: { $in: ['Offline', '$chargers.status'] },
                  then: 'Offline',
                  else: 'Online',
                },
              },
      ...
      {
        $lookup: {
          from: 'ev_models', // Assuming the collection name is ev_models
          localField: 'chargers.evModel',
          foreignField: '_id',
          as: 'evModelData',
        },
      },
      ...
      {
        $addFields: {
          capacity: '$evModelData.capacity',
          connectorTypeArray: { $ifNull: ['$evModelData.charger_type', []] },
        },
      },
      {
        $addFields: {
      ...
      },
      {
        $group: {
          ...
          connectorType: { $first: '$connectorType' },
          isBusy: { $first: '$isBusy' },
        },
      },
    ]);
    ...
    res
      .status(200)
      .json({ success: true, count: mappedData.length, result: mappedData })
  } catch (err) {
    res.status(500).json({ success: false, message: 'Server error' })
  }
}

const createPolygon = (start, end, buffer = 0.009) => {
