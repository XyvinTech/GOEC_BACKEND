...
const { sendPushNotificationToAll, sendPushNotification } = require("../services/firebaseClient");
...
exports.userNotificationList = async (req, res) => {
    ...
    let notificationList = await Notification.find({ sendTo: userId }, 'title body image createdAt').sort({createdAt:-1})
    ...
    notificationList = notificationList.map(notification => {
        ...
        formattedNotification.createdAt = moment(formattedNotification.createdAt).format('YYYY-MM-DD HH:mm:ss.SSS');
        return formattedNotification;
    });


    res.status(200).json({ message: 'Ok', result: notificationList, status: true })
}
...
exports.saveNotification = async (req, res) => {
    ...
    if (!Array.isArray(users) || !users.length) throw new Error('users should be an array');

    const notification = new Notification({ title, body, sendTo: users })
    const savedNotification = await notification.save()

    res.status(200).json({ message: 'Ok', result: savedNotification, status: true })
}
...
exports.dashboardFirebaseToAll = async (req, res) => {
    ...
    const notification = new Notification({ title: payload.title, body: payload.body, sendTo: allUsers })
    const savedNotification = await notification.save()
...
exports.dashboardFirebase = async (req, res) => {
    ...
    if (userArray) {
        ...
        const notification = new Notification({ title: subject, body: text, sendTo: userIdsArray,image: imagePath });
        const savedNotification = await notification.save()
    }

    if (failures.length > 0) {
        return res.status(207).json({ // HTTP 207 Multi-Status
